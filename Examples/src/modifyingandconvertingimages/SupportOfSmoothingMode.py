...
from aspose.imaging import Image, SmoothingMode, SizeF, FileFormat
...
print("Running example SupportOfSmoothingMode")
files = ["SmoothingTest.cdr", "SmoothingTest.cmx", "SmoothingTest.emf", "SmoothingTest.wmf", "SmoothingTest.odg", "SmoothingTest.svg"]
smoothing_modes = [SmoothingMode.ANTI_ALIAS, SmoothingMode.NONE]
out_files = []
for file_name in files:
	with Image.load(os.path.join(data_dir, file_name)) as image:
		...
		for smoothing_mode in smoothing_modes:
			output_file_name = os.path.join(get_output_dir(), "image_" + str(smoothing_mode.name) + "_" + file_name + ".png")
			vector_rasterization_options.smoothing_mode = smoothing_mode
			obj_init = PngOptions()
...
print("Finished example SupportOfSmoothingMode")
